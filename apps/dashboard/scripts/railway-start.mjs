...
if (!process.env.DATABASE_URL) {
  console.error('[kova] DATABASE_URL no está definida. Configúrala en Railway → Variables → referencia Postgres.');
} else {
  try {
    ...
    console.error('[kova] db:deploy falló; la app arrancará igual para healthcheck:', error?.message ?? error);
  }
}
