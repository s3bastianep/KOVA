    ...
    <div className="relative w-full">
      ...
      >
        ...
        <div className="px-5 py-4 border-b flex items-center justify-between gap-3" style={{ borderColor: '#F1F5F9' }}>
          <div>
            <p className="text-sm font-semibold" style={{ color: '#0F1F3D' }}>Informe comparativo</p>
            <p className="text-xs mt-0.5" style={{ color: '#3D3D3D' }}>3 candidatos evaluados</p>
          </div>
          <span className="text-[10px] font-medium px-2 py-1 rounded-md" style={{ background: '#F8FAFC', color: '#3D3D3D' }}>
            Terna lista
          </span>
        </div>
        ...
        <div className="flex border-b px-5 gap-1" style={{ borderColor: '#F1F5F9' }}>
          {tabs.map(({ id, label }) => (
            <button
              ...
              style={{
                color: tab === id ? '#1A3FAA' : '#94A3B8',
                borderBottom: tab === id ? '2px solid #2D5BE3' : '2px solid transparent',
              }}
            >
              {label}
            </button>
        ...
        <div className="p-5">
          {tab === 'resumen' && (
            <>
              <div className="flex items-end justify-between mb-5 pb-5 border-b" style={{ borderColor: '#F1F5F9' }}>
                <div>
                  <p className="text-sm font-semibold mb-1" style={{ color: '#0F1F3D' }}>María López</p>
                  <p className="text-xs mb-2" style={{ color: '#3D3D3D' }}>Mejor coincidencia con el puesto</p>
                  <p className="font-heading font-bold text-3xl leading-none tabular-nums" style={{ color: '#1A3FAA' }}>
                    {MATCH}%
                  </p>
                </div>
                ...
                {competencias.map(({ label, value }) => (
                  <div key={label}>
                    <div className="flex justify-between text-[11px] mb-1">
                      ...
                      <span className="tabular-nums" style={{ color: '#3D3D3D' }}>{value}%</span>
                    </div>
              ...
              <div className="rounded-lg px-3.5 py-3" style={{ background: '#F8FAFC', border: '1px solid #E8ECF1' }}>
                <p className="text-[10px] font-semibold uppercase tracking-wide mb-1" style={{ color: '#3D3D3D' }}>
          ...
          {tab === 'comparativo' && (
            <>
              <p className="text-xs mb-4 flex items-center gap-1.5" style={{ color: '#3D3D3D' }}>
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span
                        ...
                        style={{ background: highlight ? '#F8F9FB' : '#F1F5F9', color: highlight ? '#1A3FAA' : '#94A3B8' }}
                      >
                        {id}
                      </span>
                      <div>
                        <span className="text-xs font-medium block" style={{ color: '#334155' }}>{name}</span>
                        {tag && <span className="text-[10px]" style={{ color: '#3D3D3D' }}>{tag}</span>}
                      </div>
                    </div>
                    <span className="text-sm font-semibold tabular-nums" style={{ color: highlight ? '#1A3FAA' : '#94A3B8' }}>
                      {score}%
                    </span>
                  </div>
                ))}
          ...
          {tab === 'recomendacion' && (
            <div className="space-y-4">
              <p className="text-xs font-medium" style={{ color: '#3D3D3D' }}>Terna recomendada</p>
              <ol className="space-y-2">
                {[
                  ...
                  <li
                    ...
                    style={{
                      color: i === 0 ? '#0F1F3D' : '#3D3D3D',
                      borderColor: i === 0 ? '#2D5BE3' : '#E2E8F0',
              ...
              <p className="text-xs leading-relaxed pt-2 border-t" style={{ color: '#3D3D3D', borderColor: '#F1F5F9', lineHeight: 1.65 }}>
