...
function CandidateCard({ c }: { c: CandidateRow }) {
  ...
  return (
    <div className="kova-card kova-card-hover p-5 flex flex-col lg:flex-row lg:items-center gap-5">
      {/* Perfil */}
        ...
        <div className="min-w-0">
          ...
          {c.companyName && (
            <p className="text-xs mt-1 inline-flex items-center gap-1 font-medium" style={{ color: 'var(--kova-blue)' }}>
              <Building2 className="w-3 h-3 shrink-0" /> Postula a {c.companyName}
            </p>
          )}
      ...
      {/* Etapa + acción + scores */}
      <div className="flex-1 min-w-0 grid sm:grid-cols-[auto_1fr] gap-4 items-center">
        <div className="flex items-center gap-3">
          <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg font-medium whitespace-nowrap" style={{ background: stg.bg, color: stg.color }}>
            <StageIcon className="w-3.5 h-3.5" /> {stageLabel(c.currentStage)}
          </span>
          <div className="min-w-0">
            <p className="text-xs font-medium truncate" style={{ color: 'var(--kova-navy)' }}>{action.title}</p>
      ...
      {/* Compatibilidad */}
      <div className="flex items-center gap-4 lg:border-l lg:border-slate-100 lg:pl-5 shrink-0">
        <div className="text-center">
          <CompatRing value={compat} />
          <p className="text-[10px] mt-1 font-medium" style={{ color: scoreColor(compat) }}>{coincidencia(compat)}</p>
        </div>
...
function MiniScore({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-sm font-bold leading-none mt-0.5" style={{ color }}>{value}%</p>
